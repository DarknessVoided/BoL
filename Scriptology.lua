...
function Riven:__init()
  self.ts = TargetSelector(TARGET_NEAR_MOUSE, 450, DAMAGE_PHYSICAL, false, true)
  self:Menu()
  self.Target = Target
  self.QState = false
  self.WState = false
...
function Riven:Combo()
  ...
  if GetDistance(self.Target) > self.orbTable.trueRange + 30 and sReady[_E] and Config:getParam("Combo", "E") and GetDistance(self.Target) < data[2].range then
    if Config:getParam("Combo", "R") and self:CalcComboDmg(self.Target, 0) >= self.Target.health and myHero:GetSpellData(_R).name == "RivenFengShuiEngine" then 
      Cast(_R) 
      DelayAction(function() Cast(_E, self.Target) end, 0.075) 
    else
  ...
  if Config:getParam("Combo", "R") and (self.QCast == 2 or GetDmg(_R,myHero,self.Target)+GetDmg(_Q,myHero,self.Target)+GetDmg("AD",myHero,self.Target)+self:DmgP(self.Target,myHero.totalDamage*1.2) >= self.Target.health) and myHero:GetSpellData(_R).name ~= "RivenFengShuiEngine" then Cast(_R, self.Target) self.rNow = false end
