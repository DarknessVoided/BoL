...
-- { Global functions
  ...
  function EnemiesAroundAndFacingMe(Unit, range)
    ...
    for i=1,heroManager.iCount do hero = heroManager:GetHero(i) 
      if hero ~= nil and hero.team ~= myHero.team and hero.x and hero.y and hero.z and GetDistance(hero, Unit) < range and pos then pos, b = PredictPos(hero) if GetDistance(pos, myHero) < GetDistance(hero, myHero) then c=c+1 end end end return c
  end
