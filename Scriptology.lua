...
--[[
SAsheVersion          = 1.1 -- should be fixed
SAzirVersion          = 1
...
function OnLoad()
  ...
  champList = { "Ashe", "Blitzcrank", "Brand", "Cassiopeia", 
                ...
                "Darius", "Ekko", "Gnar", "Hecarim", "Jarvan", 
                "Kalista", "KogMaw", "LeeSin", "Lux", "Malzahar", 
                "Nidalee", "Olaf", "Orianna", "Quinn", "Rengar", 
                "Riven", "Rumble", "Sejuani", "Shyvana", "Teemo", 
                "Vayne", "Viktor", "Volibear", "Yasuo", "Yorick" }]]--
  supported = {}
...
function Vars()
  ...
  championData = {
      ...
      ["Darius"] = {
        [_Q] = { range = 0, dmgAD = function(AP, level, Level, TotalDmg, source, target) return 35*level+35+0.7*TotalDmg end},
        [_W] = { range = 0, dmgAD = function(AP, level, Level, TotalDmg, source, target) return TotalDmg+0.2*level*TotalDmg end},
        [_E] = { range = 550},
        [_R] = { range = 0, dmgTRUE = function(AP, level, Level, TotalDmg, source, target) return math.floor(70+90*level+0.75*myHero.addDamage+0.2*GetStacks(target)*(70+90*level+0.75*myHero.addDamage)) end}
      },
      ["Ekko"] = {
      ...
      ["Kalista"] = {
        [_Q] = { speed = 1750, delay = 0.25, range = 1375, width = 70, collision = true, aoe = false, type = "linear", dmgAD = function(AP, level, Level, TotalDmg, source, target) return 0-50+60*level+TotalDmg end},
  ...
  for k,enemy in pairs(GetEnemyHeroes()) do
    if ValidTarget(enemy) and enemy.visible then
      ...
      if myHero.charName == "Kalista" then
        ...
        if myHero:CanUseSpell(_R) == READY and damageR > 0 then
          killTextTable[enemy.networkID].indicatorText = killTextTable[enemy.networkID].indicatorText.."R"
        end
        if enemy.health < damageQ+damageW+damageE+damageR then
...
function Ashe:Combo()
  ...
  if Config:getParam("Combo", "R") and GetDmg(_R, myHero, Target)+GetDmg("AD", myHero, Target)*3 < Target.health then
    Cast(_R, Target, false, true, 1.9)
  end
end
...
function Cassiopeia:LaneClear()
  ...
  if myHero:CanUseSpell(_E) == READY and Config:getParam("LaneClear", "E") and Config:getParam("LaneClear", "mana", "E") <= 100*myHero.mana/myHero.maxMana then
    for minion,winion in pairs(Mobs.objects) do
      if winion ~= nil and (stackTable[winion.networkID] and stackTable[winion.networkID] > 0) then
        Cast(_E, winion, true)
      end
    end
    for minion,winion in pairs(JMobs.objects) do
...
function Cassiopeia:Combo()
  ...
  if Config:getParam("Combo", "R") and (GetDmg(_R, myHero, Target) + 2*GetDmg(_E, myHero, Target) >= Target.health or (EnemiesAround(Target, 500) > 1 and stackTable[Target.networkID] and stackTable[Target.networkID] > 0)) and ValidTarget(Target, data[3].range) then
    Cast(_R, Target, true)
  end
end
...
function Darius:Menu()
  ...
  Config:addParam({state = "LastHit", name = "LastHit", key = string.byte("X"), code = SCRIPT_PARAM_ONKEYDOWN, value = false})
  if Ignite ~= nil then Config:addParam({state = "Killsteal", name = "Ignite", code = SCRIPT_PARAM_ONOFF, value = true}) end
  Config:addParam({state = "Misc", name = "offset", code = SCRIPT_PARAM_SLICE, text = {"E"}, slider = {100}})
end
...
function Darius:CastQ(target) 
  ...
  local dist = target.ms < 350 and 0 or (Vector(myHero.x-target.x, myHero.y-target.y, myHero.z-target.z):len() < 0 and 15 or 0)
...
function LeeSin:Killsteal()
  for k,enemy in pairs(GetEnemyHeroes()) do
    if ValidTarget(enemy) and enemy ~= nil and not enemy.dead and enemy.visible then
      ...
      elseif myHero:CanUseSpell(_R) == READY and enemy.health < GetDmg(_R, myHero, enemy)*2.5 and Config:getParam("Killsteal", "R") and ValidTarget(enemy, myHero.range+myHero.boundingRadius) then
        Cast(_R, enemy, true)
      elseif Ignite and myHero:CanUseSpell(Ignite) == READY and enemy.health < (50 + 20 * myHero.level) / 5 and Config:getParam("Killsteal", "Ignite") and ValidTarget(enemy, 600) then
        CastSpell(Ignite, enemy)
      end
...
function Lux:ShieldManager(unit, spell)
  if not unit.isMe and unit.team ~= myHero.team then
    if spell.target.isMe then
      ...
      if isOnSegment and GetDistanceSqr(proj1, spell.endPos) <= (self:GetHitBox(myHero) ^ 2) then
        if Config:getParam("Misc", "Wa") and myHero:CanUseSpell(_W) == READY and myHero.health/myHero.maxHealth < 0.85 then
          Cast(_W, myHero)
        end
      end
...
function Orianna:Menu()
  ...
  Config:addParam({state = s, name = "Harrass", code = SCRIPT_PARAM_SLICE, text = {"Q","W"}, slider = {30,50}})
  Config:addParam({state = "Combo", name = "Combo", key = 32, code = SCRIPT_PARAM_ONKEYDOWN, value = false})
  Config:addParam({state = "Harrass", name = "Harrass", key = string.byte("C"), code = SCRIPT_PARAM_ONKEYDOWN, value = false})
  Config:addParam({state = "LaneClear", name = "LaneClear", key = string.byte("V"), code = SCRIPT_PARAM_ONKEYDOWN, value = false})
  Config:addParam({state = "LastHit", name = "LastHit", key = string.byte("X"), code = SCRIPT_PARAM_ONKEYDOWN, value = false})
  if Ignite ~= nil then Config:addParam({state = "Killsteal", name = "Ignite", code = SCRIPT_PARAM_ONOFF, value = true}) end
  Config:addParam({state = "Misc", name = "Ra", code = SCRIPT_PARAM_ONOFF, value = true})
end
...
function Orianna:Combo()
  ...
  if myHero:CanUseSpell(_R) == READY and Target.health < (GetDmg(_R, myHero, Target)+GetDmg("AD", myHero, Target)) and Config:getParam("Combo", "R") then
    CastR(Target)
  end
end
...
function CastW(unit)
  ...
  if GetDistance(unit, Ball) < data[1].width then  
    CastSpell(_W)
  end  
end
...
function CastR(unit)
  ...
  local Ball = objHolder["TheDoomBall"] or myHero
  if GetDistance(unit, Ball) < data[3].width then  
    CastSpell(_R)
  end  
end
...
function Rengar:LastHit()
  if myHero.mana == 5 and Config:getParam("LaneClear", "W") and (myHero.health / myHero.maxHealth) * 100 < 90 then
    ...
    if Config:getParam("LaneClear", "W") then
      ...
      if hit > 1 and minionTarget.health < GetDmg(_W, myHero, minionTarget) then
        CastSpell(_W)
      end
    end
