...
function Aimbot:Menu()
  ...
  self.Config.misc:addParam("ser",  "Which LoL version?", SCRIPT_PARAM_LIST, 1, {"5.18", "5.17", "5.16", "5.15", "5.14", "5.13", "5.12", "5.11", "5.10", "5.9", "5.8", "5.7"})
              ...
              if self:IsJayceQ(i) then 
                if myHero:CanUseSpell(_E) then 
                    ...
                    if myHero.hasMovePath then
                        local pos = myHero + Vector(Vector(Vector(myHero:GetPath(myHero.pathIndex))-myHero):normalized()*20):perpendicular()
