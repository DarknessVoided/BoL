...
function Aimbot:Vars()
    ...
    self.opcs = {{0x29, 0x93, 0xC6, 0x1F, 0x09}, {0x140, 0x22, 0x9C, 0x39, 0x2D}, {0xE4, 0x23, 0x54, 0xED, 0x82}, {0x07, 0x15, 0xB9, 0x93, 0xE4}, {0xDF, 0x46, 0xEF, 0x0E, 0xCF}, {0x50, 0xAD, 0xED, 0x8D, 0xCD}, {0x9B, 0x6B, 0x6A, 0x69, 0x68}, {0x09, 0xC8, 0xF7, 0xF2, 0x33}, {0x10B, 0x68, 0xEE, 0xB1, 0xCE}, {0x87, 0xEC, 0x6C, 0x74, 0x98}, {0x00E9, 0x02, 0xD8, 0xB3, 0xE7}}
    self.opcpos = {31, 6, 6, 14, 27, 31, 10, 27, 27, 23, 27}
...
function Aimbot:Menu()
  ...
  self.Config.misc:addParam("ser",  "Which LoL version?", SCRIPT_PARAM_LIST, 1, {"5.17", "5.16", "5.15", "5.14", "5.13", "5.12", "5.11", "5.10", "5.9", "5.8", "5.7"})
...
function Aimbot:SendPacket(p)
    if self.Config.tog and not self.Config.off and not myHero.dead and self:IsFirstCast() then
        ...
        if head == self.opcs[self.Config.misc.ser][1] then -- old: 0x00E9
            ...
            for i=0,3 do
                if opc == self.opcs[self.Config.misc.ser][i+2] and not self.toCast[i] and self.toAim[i] and self.Config.skConfig[self.str[i]..myHero.charName] > 0 then -- old: 0x02
                    self.Target = self:GetCustomTarget(i)
