...
function Ekko:Vars()
  ...
  for k=1,objManager.maxObjects,1 do
    ...
    if object and object.name and object.name == "Ekko" then
...
function Ekko:Tick()
  ...
  if self.RReady() and self.Config.casual.saveme and ((myHero.health <= myHero.maxHealth * self.Config.casual.hp/100 and self.myEvilTwin and GetDistance(self.myEvilTwin) > 800)) and EnemiesAround(myHero, 800) >= 1 then-- or (#self.hpTable>4 and self.hpTable[#self.hpTable-3][2]/myHero.maxHealth > self.Config.casual.restore/100)
    self:CastR()
  end
...
function Ekko:Draw()
  ...
  if self.Config.Drawing.R and self.myEvilTwin ~= nil and myHero:GetSpellData(_R).currentCd < 4 then
    for i=1,4 do
      LagFree(self.myEvilTwin.x, self.myEvilTwin.y, self.myEvilTwin.z, 450, 2, ARGB(75, 155, 155, 155), 3, (math.pi/(6))*(i-1))
    end 
    for i=1,4 do
      LagFree(self.myEvilTwin.x, self.myEvilTwin.y, self.myEvilTwin.z, 260, 2, ARGB(75, 155, 155, 155), 3, (math.pi/(6))*(i-1))
    end 
    LagFree(self.myEvilTwin.x, self.myEvilTwin.y, self.myEvilTwin.z, 450, 2, ARGB(75, 155, 155, 155), 12, 0)
  end
...
function Ekko:GetDmg(spell, target, source)
  ...
  elseif spell == "W" then
    APDmg = WLevel > 0 and (target.maxHealth-target.health)*(0.05+math.floor(AP/45)/100) or 0
  elseif spell == "E" then
