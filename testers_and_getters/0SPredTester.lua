...
function OnLoad() 
  	...
  	Config:addParam("hc","Hitchance", SCRIPT_PARAM_SLICE, 0, 0, 4, 1)
  	Config:addParam("spell","Spell", SCRIPT_PARAM_SLICE, 0, 0, 3, 0)
  	Config:addParam("p", "Print", SCRIPT_PARAM_ONOFF, false)
  	sts = TargetSelector(TARGET_LESS_CAST, 1200, DAMAGE_MAGIC)
end
...
function OnTick()
	...
	for _,v in pairs(GetEnemyHeroes()) do
		if v.visible and (not v.dead) then 
			if GetDistance(myHero,v) <= 1175 and GetDistance(myHero,v) >= minHitDistance and (not Target or v.health < Target.health) then Target = v end
		end
	end
	if Config.s then
		--myHero:MoveTo(mousePos.x,mousePos.z)
		if Target ~= nil and GetDistance(Target, myHero) < 1175 then
			hposh, hchance, hhpos = SP:Predict(spell, myHero, Target)
			if hchance and hchance >= Config.hc then
				print("Shootchance:"..hchance)
...
function OnDraw()
	...
	if predTarget ~= nil then
		if not Config.s then
			...
			if chance and chance > 0 then print(predTarget.charName.." Hitchance:"..chance) end
		end
